...
const FarmDetailPage = () => {
  ...
  return (
    <View className="flex-1 bg-gray-100 p-4">
      <Text className="text-3xl font-bold text-gray-800">{farmDetail?.name}</Text>
      ...
      <Text className="text-gray-600 mt-1">Description: {farmDetail?.description}</Text>
      <Text className="text-gray-600 mt-1">Rating: {farmDetail?.rating}</Text>

      {/* Farm Images Section */}
      {farmDetail?.farmImages && farmDetail.farmImages.length > 0 && (
        <View className="mt-6">
          <Text className="text-2xl font-semibold text-gray-800 mb-2">Farm Images</Text>
          <FlatList
      ...
      {farmDetail?.kois && farmDetail.kois.length > 0 && (
        <View className="mt-6">
          <Text className="text-2xl font-semibold text-gray-800 mb-2">Koi Information</Text>
          {farmDetail.kois.map((koi) => (
            <View key={koi.id} className="bg-white p-4 rounded-lg shadow mb-4">
              <Text className="text-gray-700 font-semibold">Name: {koi.name}</Text>
      ...
      {farmDetail?.trips && farmDetail.trips.length > 0 && (
        <View className="mt-6">
          <Text className="text-2xl font-semibold text-gray-800 mb-2">Trips Information</Text>
